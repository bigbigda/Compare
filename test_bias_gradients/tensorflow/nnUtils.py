...
def BinarizedAffine2(nOutputPlane, bias=True, name=None, reuse=None):
    def b_affineLayer(x, is_training=True):
        with tf.variable_op_scope([x], name, 'Affine', reuse=reuse):
            ...
            #reshaped = tf.contrib.layers.flatten(bin_x)
            nInputPlane = reshaped.get_shape().as_list()[1]
            # w = tf.get_variable('weight', [nInputPlane, nOutputPlane], initializer=tf.contrib.layers.xavier_initializer())
            w = tf.get_variable('weight', [nInputPlane, nOutputPlane],
                                initializer=tf.random_uniform_initializer(-1.,+1.))
...
def Sequential(moduleList):
    ...
    # Create model
        output = x
        for i,m in enumerate(moduleList):  # m中已经包含了除了本层输入外所需的所有的参数
            output = m(output, is_training=is_training)
            tf.add_to_collection(tf.GraphKeys.ACTIVATIONS, output)  # 用于数据可视化
        return output
    return model
