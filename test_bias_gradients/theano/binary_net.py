...
class DenseLayer(lasagne.layers.DenseLayer):
    ...
    def __init__(self, incoming, num_units, 
        ...
        if self.binary:
            super(DenseLayer, self).__init__(incoming, num_units, W=lasagne.init.Uniform((-self.H,self.H)), b=None, **kwargs)
...
class Conv2DLayer(lasagne.layers.Conv2DLayer):
    ...
    def __init__(self, incoming, num_filters, filter_size,
        ...
        if self.binary:
            super(Conv2DLayer, self).__init__(incoming, num_filters, filter_size, W=lasagne.init.Uniform((-self.H,self.H)),b=None, **kwargs)   
            # add the binary tag to weights            
            self.params[self.W]=set(['binary'])
        else:
...
def train(train_fn,val_fn,
	...
	err =0
        ...
        for i in range(batches):
            new_loss,new_err,res= train_fn(X[i*batch_size:(i+1)*batch_size],y[i*batch_size:(i+1)*batch_size],LR)
            loss += new_loss
            err  += new_err
            #print (res)
    ...
    #data[31]=data[31]*1e5
    #data[0]=np.rot90(data[0],axes=(2,3))
    #data[0]=np.rot90(data[0],axes=(2,3))
    #data[1]=np.zeros([128],dtype='float32')
    lasagne.layers.set_all_param_values(model,data[0:45])
    
    test_err, test_loss = val_epoch(X_train,y_train)
    print(" This is the init test result")
    ...
    for epoch in range(3):
        ...
        train_err,train_loss = train_epoch(X_train,y_train,10)
        #X_train,y_train = shuffle(X_train,y_train)
        ...
        #print("  training error rate:           "+str(train_err)+"%")
        ...
        #LR *= LR_decay
        if (count==1):
          np.savez("theano_data.npz", *lasagne.layers.get_all_param_values(model))

        if (count==2):
          np.savez("gb3_2.npz", *lasagne.layers.get_all_param_values(model))
