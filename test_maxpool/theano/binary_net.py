...
def train(train_fn,val_fn,
	...
	err =0
        ...
        for i in range(batches):
            new_loss,new_err,res= train_fn(X[i*batch_size:(i+1)*batch_size],y[i*batch_size:(i+1)*batch_size],10)
            loss += new_loss
            err  += new_err
            #print (res)
    ...
    data=[]
    npzfile=np.load("cifar10_parameters.npz")
    for i in range(54):
        name="arr_"+str(i)
    ...
    lasagne.layers.set_all_param_values(model,data[0:54])
